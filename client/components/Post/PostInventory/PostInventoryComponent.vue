...
const props = defineProps(["post", "id", "fibers"]);
  ...
  <section>
    ...
    <section class="fibers" v-else-if="props.fibers.length !== 0">
      <article v-for="fiber in props.fibers" :key="fiber._id">
        <PostFiberComponent :fiber="fiber" :id="props.id" @refreshFibers="getInventory" />
      </article>
    </section>
    <p v-else>No inventory</p>
    <section v-if="currentUsername===props.post.author">
      <h2>Add fiber:</h2>
      <AddPostFiberForm :id="props.id" @refreshFibers="getInventory" />
    </section>
  </section>
  <!-- <div>
